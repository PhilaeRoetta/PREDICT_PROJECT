/* The purpose of this table is to strictly copy data from curated_predict_game_check, on a incremental way
Will be run via Python program when it is manually decided in Python program input 
that curated_predict_game_check is ok */
{{config(
...
SELECT 
    * 
FROM 
    {{ref('curated_predict_game_check')}}
