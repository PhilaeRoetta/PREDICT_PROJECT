...
ranked_games AS (
    ...
    LEFT JOIN
        ...
        ON game_adj.COMPETITION_KEY = compet.COMPETITION_KEY
    LEFT JOIN 
        {{source("LAND",'GAME_MODIF')}} modif 
        ON modif.GAME_SOURCE_ID = game_adj.GAME_SOURCE_ID
        AND modif.COMPETITION_ID = compet.COMPETITION_ID
...
final_game as (
    SELECT
        ...
        {% if is_incremental() %}
            CASE
                -- we get time from the current version unless doesn't exist
                WHEN adj_game.IS_PLAYED = 1 THEN COALESCE(this.TIME_GAME_LOCAL,adj_game.TIME_GAME_LOCAL)
                ELSE adj_game.TIME_GAME_LOCAL
            END AS TIME_GAME_LOCAL,
            CASE
                -- we get time from the current version unless doesn't exist
                WHEN adj_game.IS_PLAYED = 1 THEN COALESCE(this.TIME_GAME_UTC,adj_game.TIME_GAME_UTC)
                ELSE adj_game.TIME_GAME_UTC
            END AS TIME_GAME_UTC,
        {% else %}
    ...
    LEFT JOIN
        {{this}} this
        ON this.GAME_KEY = adj_game.GAME_KEY
    {% endif %}
)
SELECT
    final_game.GAME_KEY,
