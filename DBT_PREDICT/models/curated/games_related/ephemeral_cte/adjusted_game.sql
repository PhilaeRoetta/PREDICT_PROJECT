...
--we add the utc date and time corresponding to the game date and time
game_utc as (
    SELECT
        game_message.SEASON_KEY,
        game_message.COMPETITION_KEY,
        game_message.TEAM_HOME_KEY,
        game_message.TEAM_AWAY_KEY,
        game_message.GAMEDAY_KEY,
        game_message.GAME_KEY,
        game_message.GAME_SOURCE_ID,
        game_message.GAMEDAY,
        game_message.GAMEDAY_MESSAGE,
        game_message.SCORE_HOME,
        game_message.SCORE_AWAY,
        game_message.DATE_GAME_LOCAL,
        game_message.TIME_GAME_LOCAL,
        {{local_datetime_to_utc('game_message','DATE_GAME_LOCAL', 'TIME_GAME_LOCAL', 'DATE')}} AS DATE_GAME_UTC,
        {{local_datetime_to_utc('game_message','DATE_GAME_LOCAL', 'TIME_GAME_LOCAL', 'TIME')}} AS TIME_GAME_UTC
    FROM
        game_with_message game_message
    LEFT JOIN
        {{ ref('local_time')}} lt 
        ON game_message.SEASON_COUNTRY = lt.COUNTRY
)
SELECT * FROM game_utc