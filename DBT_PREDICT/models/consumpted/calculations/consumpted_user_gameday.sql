/*  The purpose of this table is to retrieve user per gamedays who made predictions:
    ...
    Filter:
        Only gameday with action (IS_TO_DELETE or IS_TO_CALCULATE)
    Materialization:
...
with user_game as (
    SELECT 
        ...
        predict.AUTOMATIC_SCORE,
        predict.NB_PREDICTION,
...
grouped as (
    SELECT
        ...
        SUM(ug.AUTOMATIC_SCORE) AS AUTOMATIC_SCORE,
        SUM(ug.NB_PREDICTION) AS NB_PREDICTION,
...
score as (
    SELECT
        ...
        grouped.AUTOMATIC_SCORE,
        0 AS PARTICIPATION_SCORE,
        grouped.CALCULATED_SCORE AS TOTAL_SCORE,
        grouped.CALCULATED_SCORE AS POINTS,
        grouped.NB_PREDICTION,
