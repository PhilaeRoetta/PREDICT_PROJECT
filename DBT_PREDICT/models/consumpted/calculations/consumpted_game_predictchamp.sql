/*  The purpose of this table is to calculate the results of the prediction championship games
    ...
    Filter:
        Only games related to the gameday with action (IS_TO_DELETE or IS_TO_CALCULATE)
    Materialization:
...
--we extract max of team points for gameday with action
team_points as (
    SELECT
        ug.TEAM_CHOICE_KEY AS TEAM_KEY,
        gameday.GAMEDAY_KEY,
        MAX(ug.POINTS) AS TEAM_POINTS 
    FROM
...
-- we calculate home bonus points, total points per team, and winner for each game: if tied, the winner is home team
final as (
    SELECT
        game.SEASON_KEY,
        game.COMPETITION_KEY,
