...
def test_onError_final_execute_tmp_folder_cleanup_error():
    # this test the function onError_final_execute with a permission error for destroying folders
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_exit_program_decorator_catches_exception_and_exits():
    ...
    with patch("config.onError_final_execute") as mock_final:
...
def test_multithreading_run_with_failures():
    # this test the function multithreading_run with failure. SHould propagate it to all thread
    def bad_fn(x):
