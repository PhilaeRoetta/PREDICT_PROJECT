...
def test_initiate_folder():
    ...
    with patch("dropbox_actions.copy_folder") as mock_copy:
        ...
        mock_copy.assert_any_call("global_manual_inputs", "current/inputs/manual", sourcepath_from_root=1, targetpath_from_root=0, replace=0)
        mock_copy.assert_any_call("local_manual_inputs", "current/inputs/manual",replace=0)
