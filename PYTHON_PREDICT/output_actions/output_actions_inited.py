...
def create_inited_messages_for_country(param_dict: dict, country: str, template: str, sr_gameday_output_init: pd.Series) -> Tuple[str,str]:
    ...
    # ... and on condition
    content = outputA.replace_conditionally_message(output_text = content, 
                                            begin_tag = "#USER_CAN_CHOOSE_TEAM_FOR_PREDICTCHAMP_BEGIN#", 
                                            end_tag = "#USER_CAN_CHOOSE_TEAM_FOR_PREDICTCHAMP_END#", 
                                            condition = param_dict['USER_CAN_CHOOSE_TEAM_FOR_PREDICTCHAMP'] == 1)
    
    content = outputA.replace_conditionally_message(output_text = content, 
                                                begin_tag = "#REMAINING_GAMES_BEGIN#", 
                                                end_tag = "#REMAINING_GAMES_END#", 
                                                condition = param_dict['NB_GAMES_REMAINING'] > 0)   
